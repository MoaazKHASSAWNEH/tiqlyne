...
const SLIDE_IN_DIRECTIONS: ReadonlyArray<SlideInDirection> = [
  'left',
  'right',
  'top',
  'bottom'
...
function buildInitialTransform(
...
export class SlideInMotion extends BaseMotionDefinition<SlideInMotionOptions> {
  ...
  normalizeOptions(
    ...
    const initialKeyframe = context.options.fade
      ? {
          transform: buildInitialTransform(
            context.options.direction,
            context.options.distance
          ),
          opacity: 0,
          offset: 0
        }
